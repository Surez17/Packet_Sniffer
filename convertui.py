...
def convert():
	...
	for ts, buf in pcap:
	    ...
	    c.writerow(data)
	w = Label(main, text="Converted Successfully into data.csv",padx = 10)
	w.pack(side="top")

if __name__=='__main__':
    main=Tk()
    main.geometry('700x700')
    main.title('Packet Sniffer convertor')
    convert = Button(main, text=" click to convert the pcap into csv ", width=40, command=convert)
    convert.pack(padx=10, pady=100)
